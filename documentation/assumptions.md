...
The product must be available in the warehouse.
If the customer is a VMI customer, the warehouse must be allowed for that customer.
The warehouse must have an allowed port connection.
A transportation rate must exist for the candidate port, destination port, service level and weight band.
...
identifying over-capacity warehouses,
finding alternative warehouse options for selected orders,
choosing the lowest-cost alternative for each eligible order,
applying selected relocations.
